...
function ObtenerNombreProducto($identificador)
{
	...
	$row_ConsultaFuncion = mysql_fetch_assoc($ConsultaFuncion);
	$totalRows_ConsultaFuncion = mysql_num_rows($ConsultaFuncion);
	$nombre=$row_ConsultaFuncion['strNombre'];
	return $nombre; 
	mysql_free_result($ConsultaFuncion);
}
...
//***************************************************
//***************************************************
function fn_ObtenerCorreodeidSucursal($identificador)
{

	global $database_conexionproductos, $conexionproductos;
	mysql_select_db($database_conexionproductos, $conexionproductos);
	$query_ConsultaFuncion = sprintf("SELECT strEmailNotificacion FROM tblcakestore WHERE idPasteleria = %s", $identificador);
	//echo $query_ConsultaFuncion;
	$ConsultaFuncion = mysql_query($query_ConsultaFuncion, $conexionproductos) or die(mysql_error());
	$row_ConsultaFuncion = mysql_fetch_assoc($ConsultaFuncion);
	$totalRows_ConsultaFuncion = mysql_num_rows($ConsultaFuncion);
	
	return $row_ConsultaFuncion['strEmailNotificacion']; 
	mysql_free_result($ConsultaFuncion);
}
/////////////////////////////////////////////////
/////////////////////////////////////////////////
function Mostrar_Carrito_Usuario($identificador)
{
	...
	echo $row_ConsultaFuncion['idProducto']." usuario:".$row_ConsultaFuncion['idUsuario']." idCOmpra:".$row_ConsultaFuncion['intContador']; 
	echo "<br>";
...
function ObtenerProductoRelacionado($identificador)
{
	...
	return  $row_ConsultaFuncion['intProductoRelacionado1']; 
	mysql_free_result($ConsultaFuncion);
}
...
function EnvioCorreoHTML3($destinatario, $contenido, $asunto, $pastelero)
{
...
<table  width="100%" border="0" cellspacing="3" cellpadding="3">
  ...
  <tr >
  <td colspan="4" style="font-family:Arial, Helvetica, sans-serif; height:50px; font-size:12px;color:red;text-align:center;"><h3>Verifica que tu producto se encuentre en perfectas condiciones al momento de recibirlo. No aplicar&aacute; la pol&iacute;tica de garant&iacute;a una vez entregado y aceptado. </h3>
</td>
  </tr>
</table>
</body>
